...
vector<pt> convex_hull(vector<pt> v) { // convex hull - O(n log(n))
	...
	vector<pt> l, u;
	for (int i = 0; i < v.size(); i++) {
		while (l.size() > 1 and !ccw(l.end()[-2], l.end()[-1], v[i]) and not col(l.end()[-2],l.end()[-1],v[i]))
			l.pop_back();
		l.push_back(v[i]);
	}
	for (int i = v.size() - 1; i >= 0; i--) {
		while (u.size() > 1 and !ccw(u.end()[-2], u.end()[-1], v[i]) and not col(u.end()[-2],u.end()[-1],v[i]))
			u.pop_back();
		u.push_back(v[i]);
	}
...
int main() { _
	...
	v = convex_hull(v);
	cout << v.size() << '\n';
	for(ull i=0;i<v.size();i++) cout << v[i].x << ' ' << v[i].y << '\n';
