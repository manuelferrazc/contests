...
int main() { _
    ...
    for(int i=0;i<h;i++) {
        int x;
        cin >> x;
        m.insert(pair(tnc[i],x));
    }

    seg_pa st(n);

    for(auto it = m.begin();it->ff!=n-1;it++) {
        int l = it->ff+1;
        int r = next(it)->ff;

        st.update_set(l,r,(r-l+1)*it->ss,-it->ss);
    }
    ...
    while(q--) {
        ...
        if(op==1) {
            a--; // adicionar porto em a com valor b

            m[a] = b;
            auto it = prev(m.lower_bound(a));
