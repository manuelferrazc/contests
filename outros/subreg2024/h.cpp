...
bool div(string &n, string &d) {
    bint a;
	int b=0;
    
    for(int i=0;i<n.size();i++) if(n[i]=='1') a+=pot[n.size()-i-1];
    for(int i=0;i<d.size();i++) if(d[i]=='1') b+=pot2[d.size()-i-1];
	// cout << a << ' ' << b << '\n';
	if(b==0) return false;
...
void brute(string &n, string &d, vector<pair<int,int>> &v, int i) {
    if(i==v.size()) {
        if(div(n,d)) {
...
int main() { _
    pot.push_back(1);
    pot2.push_back(1);
    while(pot.size()<500) pot.push_back(bint(pot.back()*2)); 
    while(pot2.size()<25) pot2.push_back(pot2.back()*2); 
	// for(auto &i:pot) cout << i << '\n';
	// cout << '\n';
    string n,d;
    cin >> n >> d;
    ...
    for(int i=0;i<n.size();i++) if(n[i]=='*') v.push_back(make_pair(0,i));
    for(int i=0;i<d.size();i++) if(d[i]=='*') v.push_back(make_pair(1,i));
