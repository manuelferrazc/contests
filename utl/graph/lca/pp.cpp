...
int main() { _
    ...
    while(q--) {
        ...
        int mi = c[0];
        for(int i=1;i<k;i++) if(d[c[i]]>d[mi]) mi = c[i];

        int m2 = -1;
        for(int i=0;i<k;i++) {
            if(getLCA(mi,c[i])!=c[i]) {
                m2 = i;
                break;
            }
        }
        ...
        for(int i = m2+1;i<k;i++) 
            if(getLCA(c[i],c[m2])==c[m2]) m2 = i;
        
        int anc = getLCA(mi,c[m2]);
        bool ok=true;
        for(int i=0;i<k;i++) {
            if(d[c[i]]<d[anc]) {
                ok = false;
            ...
            if(getLCA(mi,c[i])!=c[i] and getLCA(c[i],c[m2])!=c[i]) {
                ok = false;
                break;
            }
        }
